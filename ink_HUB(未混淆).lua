...
local TestTab = D:Tab({Title="被遗弃(测试)", Icon="ghost"})
    ...
    Callback = function(state)
        ...
        if state then
            pcall(function()
                ...
                if sprintModule then sprintModule.StaminaLossDisabled = true end
            end)
            ...
            if staminaMonitor then staminaMonitor:Disconnect(); staminaMonitor = nil end
            pcall(function()
                if sprintModule then sprintModule.StaminaLossDisabled = false end
            end)
        end
    end
})
...
TestTab:Button({
    ...
    Callback = function()
        pcall(function()
            ...
            local sprintMod = require(SprintingModule)
            if sprintMod and sprintMod.SetStamina then
                sprintMod.SetStamina(sprintMod.MaxStamina or 100)
            end
        end)
    end
})
    ...
    Callback = function(state)
        ...
        if state then
            ...
            slashConnection = RunService.Heartbeat:Connect(function()
                if not slashEnabled then return end
                local char = LocalPlayer.Character
                if not char then return end
                ...
                for _, killer in ipairs(killers:GetChildren()) do
                    ...
                    if kRoot and (pos - kRoot.Position).Magnitude <= 10 then
                        RemoteEvent:FireServer("UseActorAbility", "Slash")
                        break
                    end
                end
            end)
        elseif slashConnection then
...
local shedAimbotEnabled = false
local shedloop = nil

TestTab:Toggle({
    Title = "Shedletsky自动瞄准",
    Value = false,
    Callback = function(state)
        shedAimbotEnabled = state
        if state then
            if LocalPlayer.Character and LocalPlayer.Character.Name ~= "Shedletsky" then
                return
            end
            shedloop = LocalPlayer.Character.Sword.ChildAdded:Connect(function(child)
                if not shedAimbotEnabled then return end
                if child:IsA("Sound") and (child.Name == "rbxassetid://12222225" or child.Name == "83851356262523") then
                    local killers = workspace:FindFirstChild("Players") and workspace.Players:FindFirstChild("Killers")
                    if killers then
                        local killer = killers:FindFirstChildOfClass("Model")
                        if killer and killer:FindFirstChild("HumanoidRootPart") then
                            local kHRP = killer.HumanoidRootPart
                            local pHRP = LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                            if pHRP and (kHRP.Position - pHRP.Position).Magnitude <= 30 then
                                for i = 1, 100 do
                                    task.wait(0.01)
                                    workspace.CurrentCamera.CFrame = CFrame.new(workspace.CurrentCamera.CFrame.Position, kHRP.Position)
                                    pHRP.CFrame = CFrame.lookAt(pHRP.Position, kHRP.Position)
                                end
                            end
                        end
                    end
...
local chancePrediction = 1
local chanceConnection = nil
local chanceTargets = {"Jason", "c00lkidd", "JohnDoe", "1x1x1x1", "Noli"}
local trackedAnims = {
    ["103601716322988"] = true, ["133491532453922"] = true, ["86371356500204"] = true,
    ["76649505662612"] = true, ["81698196845041"] = true
}
local chanceState = {triggered = false, lastTrigger = 0, duration = 1.7, maxDist = 50}

TestTab:Toggle({
    Title = "Chance自动瞄准",
    Value = false,
    Callback = function(state)
        chanceAimEnabled = state
        if state then
            if chanceConnection then chanceConnection:Disconnect() end
            chanceConnection = RunService.RenderStepped:Connect(function()
                if not chanceAimEnabled then return end
                local char = LocalPlayer.Character
                if not char then return end
                ...
                for _, track in ipairs(hum:GetPlayingAnimationTracks()) do
                    local id = track.Animation and track.Animation.AnimationId and track.Animation.AnimationId:match("%d+")
                    if id then playing[id] = true end
                end
                local triggered = false
                for id in pairs(trackedAnims) do if playing[id] then triggered = true; break end end
                if triggered then chanceState.lastTrigger = tick(); chanceState.triggered = true end
                if chanceState.triggered and tick() - chanceState.lastTrigger <= chanceState.duration then
                    local killers = workspace:FindFirstChild("Players") and workspace.Players:FindFirstChild("Killers")
                    if killers then
                        for _, name in ipairs(chanceTargets) do
                            local target = killers:FindFirstChild(name)
                            if target and target:FindFirstChild("HumanoidRootPart") then
                                local tHRP = target.HumanoidRootPart
                                if (hrp.Position - tHRP.Position).Magnitude <= chanceState.maxDist then
                                    local predicted = tHRP.Position + (tHRP.CFrame.LookVector * chancePrediction)
                    ...
                    chanceState.triggered = false
                end
            end)
        elseif chanceConnection then
...
local coin3Enabled = false
local coin3Thread = nil
...
TestTab:Toggle({
    ...
    Callback = function(state)
        coin3Enabled = state
        if state then
            coin3Thread = task.spawn(function()
                while coin3Enabled do
                    local charges = LocalPlayer.PlayerGui:FindFirstChild("MainUI") and
                                    LocalPlayer.PlayerGui.MainUI:FindFirstChild("AbilityContainer") and
                                    LocalPlayer.PlayerGui.MainUI.AbilityContainer:FindFirstChild("Shoot") and
                                    LocalPlayer.PlayerGui.MainUI.AbilityContainer.Shoot:FindFirstChild("Charges")
                    if charges and charges:IsA("TextLabel") and charges.Text == "3" then
                    else
        ...
        elseif coin3Thread then
            task.cancel(coin3Thread)
            coin3Thread = nil
        end
    end
})

local coin1Enabled = false
local coin1Thread = nil
...
TestTab:Toggle({
    ...
    Callback = function(state)
        coin1Enabled = state
        if state then
            coin1Thread = task.spawn(function()
                while coin1Enabled do
                    local charges = LocalPlayer.PlayerGui:FindFirstChild("MainUI") and
                                    LocalPlayer.PlayerGui.MainUI:FindFirstChild("AbilityContainer") and
                                    LocalPlayer.PlayerGui.MainUI.AbilityContainer:FindFirstChild("Shoot") and
                                    LocalPlayer.PlayerGui.MainUI.AbilityContainer.Shoot:FindFirstChild("Charges")
                    if charges and charges:IsA("TextLabel") and charges.Text == "1" then
                    else
                        RemoteEvent:FireServer("UseActorAbility", "CoinFlip")
                    end
                    task.wait()
        ...
        elseif coin1Thread then
            task.cancel(coin1Thread)
            coin1Thread = nil
        end
    end
})
    ...
    Callback = function(state)
        ...
        if state then
            blockConnection = RunService.Stepped:Connect(function()
                ...
                local hrp = char:FindFirstChild("HumanoidRootPart")
                if not hrp then return end
                local killers = workspace:FindFirstChild("Killers") or (workspace:FindFirstChild("Players") and workspace.Players:FindFirstChild("Killers"))
                if not killers then return end
                for _, killer in ipairs(killers:GetChildren()) do
                    local kRoot = killer:FindFirstChild("HumanoidRootPart")
                    if kRoot and (hrp.Position - kRoot.Position).Magnitude <= blockDistance then
                        local hasSound = false
                        for _, s in ipairs(kRoot:GetDescendants()) do
                            if s:IsA("Sound") and s.IsPlaying then
                                for _, id in ipairs(targetSounds) do
                                    if s.SoundId == id then hasSound = true; break end
                                end
                            end
                            if hasSound then break end
                        end
                        if hasSound and tick() - lastBlockTime >= 0.35 then
...
local twoTimeEnabled = false
local twoTimeMaxDist = 50
local twoTimeLoop = nil
local twoTimeSounds = {"rbxassetid://86710781315432", "rbxassetid://99820161736138"}

TestTab:Toggle({
    Title = "TwoTime自动瞄准(冲锋)",
    Value = false,
    Callback = function(state)
        twoTimeEnabled = state
        if state then
            if LocalPlayer.Character and LocalPlayer.Character.Name ~= "TwoTime" then return end
            twoTimeLoop = LocalPlayer.Character.HumanoidRootPart.ChildAdded:Connect(function(child)
                if not twoTimeEnabled then return end
                for _, s in pairs(twoTimeSounds) do
                    if child.Name == s then
                        local survivors = {}
                        for _, plr in ipairs(Players:GetPlayers()) do
                            if plr ~= LocalPlayer and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
                                table.insert(survivors, plr.Character)
                            end
                        end
                        local nearest = nil
                        ...
                        if pHRP then
                            for _, surv in ipairs(survivors) do
                                local sHRP = surv.HumanoidRootPart
                                local dist = (sHRP.Position - pHRP.Position).Magnitude
                                if dist < nearestDist and dist <= twoTimeMaxDist then
                                    nearestDist = dist
                                    nearest = surv
                                end
                            end
                        ...
                        if nearest then
                            ...
                            local pHRP2 = LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                            if pHRP2 then
                                local maxIter = child.Name == "rbxassetid://79782181585087" and 220 or 100
                                for i = 1, maxIter do
                                    task.wait(0.01)
                                    workspace.CurrentCamera.CFrame = CFrame.new(workspace.CurrentCamera.CFrame.Position, nHRP.Position)
                                    pHRP2.CFrame = CFrame.lookAt(pHRP2.Position, nHRP.Position)
                                end
                            end
                        end
                    end
        ...
        elseif twoTimeLoop then
            twoTimeLoop:Disconnect()
            twoTimeLoop = nil
        end
    end
})
...
local backstabMode = "背后"
local backstabCooldown = false
local backstabLastTarget = nil
...
local function deletePopups()
    ...
    if tempUI then
        for _, popup in ipairs(tempUI:GetChildren()) do
            if popup.Name == "1x1x1x1Popup" then popup:Destroy() end
        end
    end
end
...
TestTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state then
            ...
            popupConn = tempUI.ChildAdded:Connect(function(child)
                if popupEnabled and child.Name == "1x1x1x1Popup" then
                    task.defer(function() child:Destroy() end)
                end
            end)
            popupTask = task.spawn(function()
                while popupEnabled do
                    deletePopups()
                    pcall(function() RemoteEvent:FireServer("Entangled") end)
                    task.wait(0.5)
                end
            end)
        else
            if popupTask then task.cancel(popupTask); popupTask = nil end
...
RunService.Heartbeat:Connect(function()
    if not backstabEnabled or backstabCooldown then return end
    local char = LocalPlayer.Character
    if not char then return end
    local hrp = char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end
    ...
    for _, name in ipairs(killerNames) do
        local killer = killers:FindFirstChild(name)
        if killer and killer:FindFirstChild("HumanoidRootPart") then
            local kHRP = killer.HumanoidRootPart
            local dist = (hrp.Position - kHRP.Position).Magnitude
            if dist <= backstabRange and killer ~= backstabLastTarget then
                local behind = false
                if backstabMode == "周围" then
                    behind = true
                else
                    local dir = -kHRP.CFrame.LookVector
                    behind = (hrp.Position - kHRP.Position):Dot(dir) > 0.5
                end
                if behind then
                    backstabCooldown = true
                    backstabLastTarget = killer
                    hrp.CFrame = CFrame.lookAt(hrp.Position, kHRP.Position)
                    RemoteEvent:FireServer("UseActorAbility", "Dagger")
                    task.delay(1, function()
                        backstabLastTarget = nil
                        backstabCooldown = false
                    end)
                    break
                end
            end
