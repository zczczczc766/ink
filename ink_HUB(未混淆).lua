...
local CheatTab = D:Tab({Title="别被抓到作弊", Icon="shield"})
...
local R_State = Remotes:WaitForChild("ClassroomPlayerStateUpdate")
local R_Progress = Remotes:WaitForChild("ClassroomProgressUpdate")
local R_View = Remotes:WaitForChild("ClassroomViewState")
local R_Look = Remotes:WaitForChild("ClassroomLookUpdate")
local R_Caught = Remotes:WaitForChild("ForceCaughtCamera")
local R_Ad = Remotes:WaitForChild("ClassroomAdSlot")
...
local antiCaughtEnabled = false
local spoofStateEnabled = false
local spoofProgressEnabled = false
local spoofViewEnabled = false
local injectEnabled = false
local upvalueEnabled = false
local adSkipEnabled = false
...
local function notify(msg)
    A:SetCore("SendNotification",{Title="作弊助手", Text=msg, Duration=2})
end

local function getGoalAnswers()
...
local function getAnswersWritten()
    if not hasGetupvalues then return 0 end
    local ok, ups = pcall(getupvalues, function() end)
...
local function updateStat(...)
    pcall(function()
        game:GetService("ReplicatedStorage"):WaitForChild("Functions"):WaitForChild("UpdateStat"):InvokeServer(...)
    end)
end

CheatTab:Section({ Title = "防护系统" })

CheatTab:Toggle({
    Title = "反被抓系统",
    Value = false,
    Callback = function(state)
        antiCaughtEnabled = state
        if state then
            if hasHookmeta then
                pcall(function()
                    hookmetamethod(game, "__namecall", function(self, ...)
                        if not antiCaughtEnabled then return nil end
                        local method = getnamecallmethod()
                        if self == LocalPlayer and method == "GetAttribute" then
                            local key = select(1, ...)
                            if key == "TeacherCaughtCameraActive" or key == "TeacherObservationCameraActive" then
                                return false
                            end
                        end
                        return nil
            ...
            for _, attr in ipairs({"TeacherCaughtCameraActive", "TeacherObservationCameraActive"}) do
                LocalPlayer:GetAttributeChangedSignal(attr):Connect(function()
                    if antiCaughtEnabled and LocalPlayer:GetAttribute(attr) == true then
                        LocalPlayer:SetAttribute(attr, false)
                    end
                end)
            end
            task.spawn(function()
                while antiCaughtEnabled do
                    for _, attr in ipairs({"TeacherCaughtCameraActive", "TeacherObservationCameraActive"}) do
...
CheatTab:Toggle({
    Title = "StateUpdate 拦截",
    Value = false,
    Callback = function(state)
        spoofStateEnabled = state
        if state and hasHookmeta then
            pcall(function()
                hookmetamethod(game, "__namecall", function(self, method, ...)
                    if not spoofStateEnabled then return nil end
                    if method == "FireServer" and rawequal(self, R_State) then
                        local args = {...}
                        if args[2] == "SEARCHING" or args[2] == "WRITING" then
                            return nil
                        end
                    end
                    return nil
            ...
            notify("StateUpdate拦截已启用")
        else
            notify("StateUpdate拦截已关闭")
        end
    end
})

CheatTab:Toggle({
    Title = "ProgressUpdate 篡改",
    Value = false,
    Callback = function(state)
        spoofProgressEnabled = state
        if state and hasHookmeta then
            pcall(function()
                hookmetamethod(game, "__namecall", function(self, method, ...)
                    if not spoofProgressEnabled then return nil end
                    if method == "FireServer" and rawequal(self, R_Progress) then
                        local args = {...}
                        if type(args[2]) == "table" then
                            args[2].cheatSeconds = 0
                            args[2].examSeconds = 0
                        end
                    end
                    return nil
                end)
            end)
            notify("ProgressUpdate篡改已启用")
        else
            notify("ProgressUpdate篡改已关闭")
        end
    end
})

CheatTab:Toggle({
    Title = "视角伪装 (CHEAT→FREE)",
    Value = false,
    Callback = function(state)
        spoofViewEnabled = state
        if state and hasHookmeta then
            pcall(function()
                hookmetamethod(game, "__namecall", function(self, method, ...)
                    if not spoofViewEnabled then return nil end
                    if method == "FireServer" then
                        if rawequal(self, R_Look) then
                            local args = {...}
                            if args[2] and args[2] ~= "FREE" then args[2] = "FREE" end
                        end
                        if rawequal(self, R_View) then
                            local args = {...}
                            if args[2] and args[2] ~= "FREE" then args[2] = "FREE" end
                        end
                    end
        ...
        if state then
            ...
            task.spawn(function()
                while injectActive do
                    if isInRound() then
                        local written = getAnswersWritten()
                        local goal = getGoalAnswers()
                        local cleanData = {
                        ...
                        pcall(function() R_Progress:FireServer(cleanData) end)
                    end
                    task.wait(0.15)
                end
            end)
...
CheatTab:Toggle({
    ...
    Value = false,
    Callback = function(state)
        upvalueEnabled = state
        if state and hasGetupvalues and hasSetupvalue then
            upvalueActive = true
            task.spawn(function()
                            ...
                            if type(up) == "function" then
                                ...
                                if ok2 and ups2 then
                                    for j, up2 in ipairs(ups2) do
                                        if type(up2) == "number" and up2 ~= 0 then
                                            pcall(setupvalue, up, j, 0)
                                        end
                                    end
                                end
                            end
...
CheatTab:Toggle({
    ...
    Callback = function(state)
        adSkipEnabled = state
        if state then
            R_Ad.OnClientEvent:Connect(function(action, slotId)
                if adSkipEnabled and action == "Granted" then
                    task.wait(0.1)
...
CheatTab:Section({ Title = "全自动模式" })

CheatTab:Toggle({
    Title = "全自动模式",
    Value = false,
    Callback = function(state)
        fullAutoEnabled = state
        if state then
            autoRunning = true
            task.spawn(function()
                notify("全自动已启动")
                while autoRunning do
                    if not isInRound() then task.wait(1) continue end
                    local written = getAnswersWritten()
                    local goal = getGoalAnswers()
...
CheatTab:Button({
    ...
    Callback = function()
        task.spawn(function()
            if not isInRound() then notify("不在回合中") return end
            local ans = nil
            pcall(function()
                local ok, ups = pcall(getupvalues, function() end)
                if ok and ups then ans = ups[5] end
            end)
            if not ans then
                local answers = {"A", "B", "C", "D"}
                ans = answers[math.random(1, 4)]
...
task.spawn(function()
    ...
    while true do
        ...
        local written = getAnswersWritten()
        local goal = getGoalAnswers()
        local statusPara = CheatTab:GetParagraph("📊 实时状态")
        if statusPara then
            statusPara:SetDesc(
                "回合: " .. (inRound and "进行中" or "等待中") ..
                "\n已答: " .. written .. "/" .. goal ..
                "\n循环次数: " .. autoCycles
            )
        end
...
CheatTab:Section({ Title = "诊断" })

CheatTab:Paragraph({
    Title = "🔧 执行器诊断",
    Desc = "hookmetamethod: " .. (hasHookmeta and "✅" or "❌") ..
    "\nhookfunction: " .. (hasHookfunc and "✅" or "❌") ..
    "\ngetconnections: " .. (hasGetconns and "✅" or "❌") ..
    "\ngetupvalues: " .. (hasGetupvalues and "✅" or "❌") ..
    "\nsetupvalue: " .. (hasSetupvalue and "✅" or "❌"),
    Image = "activity",
    ImageSize = 24,
})
