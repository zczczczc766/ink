...
local SuperStoreTab = D:Tab({Title="超级商店", Icon="shopping-bag"})
...
SuperStoreTab:Section({ Title = "传送功能" })

SuperStoreTab:Dropdown({
    Title = "选择传送地点",
    Values = { "超市外", "电力室", "经理" },
    Value = "超市外",
    Callback = function(v)
...
SuperStoreTab:Button({
    ...
    Callback = function()
        ...
        if currentLoc == "经理" then
            ...
            if not target then
                A:SetCore("SendNotification",{Title="未找到", Text="地图中未发现经理", Duration=2})
                return
            end
        end
        ...
        savedPos = hrp.CFrame
        hrp.CFrame = target
        A:SetCore("SendNotification",{Title="已传送", Text="已传送到 " .. currentLoc, Duration=2})
    end
})
...
SuperStoreTab:Button({
    ...
    Callback = function()
        ...
        if savedPos then
            hrp.CFrame = savedPos
            savedPos = nil
            A:SetCore("SendNotification",{Title="已返回", Text="已传送回原位置", Duration=2})
        else
            A:SetCore("SendNotification",{Title="无记录", Text="没有可返回的位置", Duration=2})
        end
    end
})
...
SuperStoreTab:Section({ Title = "物品ESP" })

SuperStoreTab:Toggle({
    Title = "开启物品ESP",
    Value = false,
    Callback = function(v)
        ESPEnabled = v
...
for _, cat in ipairs(Categories) do
    SuperStoreTab:Section({ Title = cat.title })
...
SuperStoreTab:Button({
    ...
    Callback = function()
        ...
        if nearest then
            ...
            A:SetCore("SendNotification",{Title="传送成功", Text="已传送到最近物品", Duration=2})
        else
            A:SetCore("SendNotification",{Title="未找到", Text="未发现勾选物品", Duration=2})
        end
    end
})

SuperStoreTab:Section({ Title = "员工ESP" })
...
SuperStoreTab:Section({ Title = "已汉化员工" })
SuperStoreTab:Paragraph({ Title = "BuffEmployee → 强化员工", Desc = "" })
SuperStoreTab:Paragraph({ Title = "Employee → 普通员工", Desc = "" })
SuperStoreTab:Paragraph({ Title = "Roach → 蟑螂", Desc = "" })
SuperStoreTab:Paragraph({ Title = "Manager → 经理", Desc = "" })
